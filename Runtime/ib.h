...
typedef struct //_USB_DATA_
{
	struct usbDeviceInfo* _dev;
	uint8_t  _interval;
	uint16_t _maxpkt;
	uint32_t _pipeRcv;
	uint32_t _pipeSnd;
	uint32_t _pipeBlkRcv;	//01
	uint32_t _pipeBlkSnd;	//82
	uint32_t _pipeIntRcv;	//85

	int32_t  udev;
	uint32_t	usec_timeout;
	uint8_t		eos_mode;
	uint8_t		eos_char;
	RTHANDLE _usb_thread;
	int master ;
	int online;
	short			id;
	unsigned int	ibsta;
...
//	struct usb_data newU;		//data passing
//	struct usb_data oldU;


static inline  Addr4882_t MakeAddr( unsigned int pad, unsigned int sad )
{
	Addr4882_t address;
...
static inline unsigned int GetPAD( Addr4882_t address )
{
	return address & 0xff;
}

static inline  unsigned int GetSAD( Addr4882_t address )
{
